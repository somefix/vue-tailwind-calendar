...
import CalendarActions from '@/components/CalendarActions.vue'
...
<template>
    ...
    <calendar-actions
      :displayed-data="calendar.displayedMonth.format('MMMM YYYY')"
      @click-prev="previous(calendar)"
      @click-next="next(calendar)"
    />
    <calendar-weeks :weeks="weeks" />
    <connected-calendar-days :displayed-month="calendar.displayedMonth" :selected-date="calendar.selectedDate" />
	</div>
</template>
